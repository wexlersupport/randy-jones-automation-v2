<script setup lang="ts">
    ...
    const meetingUuid = route.params.id as string
    ...
    onMounted(async () => {
        ...
        // const data = await fetch('/api/zoom/save-summary', {
        //     method: 'POST',
        //     body: JSON.stringify({
        //       summaryData: {
        //         meeting_host_id: response.meeting_host_id,
        //         meeting_host_email: response.meeting_host_email,
        //         meeting_uuid: response.meeting_uuid,
        //         meeting_id: response.meeting_id,
        //         meeting_topic: response.meeting_topic,
        //         meeting_start_time: response.meeting_start_time,
        //         meeting_end_time: response.meeting_end_time,
        //         summary_start_time: response.summary_start_time,
        //         summary_end_time: response.summary_end_time,
        //         summary_title: response.summary_title,
        //         summary_overview: response.summary_overview,
        //         summary_doc_url: response.summary_doc_url,
        //         summary_details: response.summary_details,
    ...
    async function getOtherData(meeting_end_time: any) {
        const nextMeetingDate = getRandomDayFromNext7()
        const today = new Date();
        const { response: next_meeting } = await getNextMeeting(nextMeetingDate)
        console.log('Next Meeting Date:', nextMeetingDate, next_meeting)
        const formatted = today.toLocaleDateString('en-US', { month: '2-digit', day: '2-digit' });
        let temp: any = new Date(meeting_end_time);
        temp = temp.toLocaleDateString('en-US', { month: '2-digit', day: '2-digit' });
        let add_note_date = `${formatted} - Did QC Meeting last ${temp} \n`
        add_note_date += `${formatted} - Set IM & ASM - ${next_meeting.choices[0].message.content} \n`;
        add_note_date += `************************************************************* \n\n`
        const marked_as_ai = ` \n\n***This was generated by AI***`
        return { add_note_date, marked_as_ai, nextMeetingDate: temp  };
    }

    async function getMeetingDetail() {
      try {
        let res: any = {}
        let summary_overview: { choices?: { message: { content?: string } }[] } = {}
        const checkResponse = await fetch(`/api/zoom/summary?meeting_uuid=${meetingUuid}`, {
            method: 'GET'
        })
        ...
        if (!check.success && check.message?.includes('No record found')) {
          toast.add({
          ...
          summary_overview = await generateSummary()

          const otherData = await getOtherData(res.response.meeting_end_time);
          form.value.meeting_host_email = res.response.meeting_host_email;
          form.value.summary_title = res.response.summary_title;
          form.value.summary_overview = res.response.summary_overview + ` Next meeting date is ${otherData.nextMeetingDate}`;
          let content = summary_overview.choices?.[0]?.message?.content?.trim() || ''
          if (content.length < 10) {
              content = form.value.summary_overview;
          }
          form.value.ai_summary_overview = otherData.add_note_date + content + otherData.marked_as_ai;

        }
        else{
          res = check
          toast.add({
            title: 'Database Record Found',
          ...
          form.value.summary_title = res.response.summary_title;
          form.value.summary_overview = res.response.summary_overview + ` Next meeting date is ${otherData.nextMeetingDate}`;
          form.value.ai_summary_overview = otherData.add_note_date + res.response.summary_content + otherData.marked_as_ai;
    ...
    async function getNextMeeting(nextMeetingDate: string) {
        ...
        const res = await response.json()
        console.log('getNextMeeting response:', res)
        return res
    }
